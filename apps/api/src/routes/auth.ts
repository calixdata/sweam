...
authRoutes.post('/signup', async (c) => {
  ...
  const user: SessionUser = { id, email: body.email, displayName: body.displayName, handle: null };
...
authRoutes.post('/signin', async (c) => {
  ...
  const row = await c.env.DB.prepare(
    ...
    .bind(body.email)
    .first<{ id: string; email: string; display_name: string; password_hash: string; handle: string | null }>();
