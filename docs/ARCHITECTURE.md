...
**Monetization eligibility** (v0.7) gates the creator share, not the ads: ads run on any published title and the platform records the gross, but the creator's 55% accrues only while they meet the published thresholds (followers, lifetime watch time on published titles, published title count, account standing), evaluated live at every serve by [apps/api/src/lib/monetize.ts](../apps/api/src/lib/monetize.ts) against the pure decision function in the shared package. The thresholds, their competitor benchmarks, and the growth path are the subject of [docs/CREATOR-PROGRAM.md](CREATOR-PROGRAM.md); the earnings page shows per-check progress from the same evaluation. Strikes pause accrual through the standing check; nothing is retroactive in either direction.

**Submissions** are the curated intake door beside the Studio's self-serve one: signed-in users pitch finished work with an https screener link and a rights confirmation (a literal-true schema field), rate limited; admins review with a note that is shared verbatim with the submitter on either decision.
