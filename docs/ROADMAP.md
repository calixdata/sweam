...
v0.1 (this repo) is the working vertical slice: catalog, discovery, playback, Studio, uploads, and range serving. The phases below are ordered by what each one unlocks.

## v0.2: Real media pipeline
...
## v0.3: The scout portal

The feature that closes the loop on the platform thesis: a read-only, ranked view for networks and studios.

- Momentum leaderboards (finish-rate leaders, fastest-growing, genre breakouts)
- Per-title one-sheet: stats over time, audience retention curve, creator contact gate
- Creator-controlled visibility (opt in, and creators see who viewed their one-sheet)
- Creator analytics dashboard sharing the same retention data
