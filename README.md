...
- **API** ([apps/api](apps/api)): Cloudflare Worker (Hono + TypeScript) with D1 (SQLite) and R2.
  ...
  - **Community** (v0.6): title comments with one level of replies and a three-role moderation chain (author delete, creator remove, admin remove; removals are soft-deletes that keep threads intact and always notify the author); comment reports feeding the admin queue; creator follows with public creator pages; a "From creators you follow" rail on Home; and follower notifications on first publishes and new episodes
  - **The Creator Program** (v0.7): a published, research-grounded monetization policy ([docs/CREATOR-PROGRAM.md](docs/CREATOR-PROGRAM.md)) benchmarked against YouTube, TikTok, Meta, Twitch, Tubi, and Netflix; monetization eligibility (followers, watch time, published titles, standing) enforced at every ad serve with live progress on the earnings page; and a public **Submit** intake where anyone can pitch finished work for human review, with decisions notifying the submitter
